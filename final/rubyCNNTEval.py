...
def evaluate(embed, testData, config):
    ...
    with tf.Session() as sess:
        ...
        totalNum = 0
        for batchNum, batches in enumerate(utils.get_minibatches(testData, config["batchSize"], False)):
            feedDict = {
                comments: batches[0],
                masks: batches[1],
                labels: batches[5],
            ...
            batchConfusionMatrix, batchAccuracy = sess.run([confusionMatrix, accuracy], feedDict)
    ...
    # Return series.
    return epochAccuracy, epochConfusionMatrix
...
if __name__ == "__main__":
    ...
    testData = utils.loadComments(args.inDir + "/ProcessedTest", float('inf'), config)
