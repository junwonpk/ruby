...
def doWork(embed, trainComments, trainCommentfs, trainLabels, devComments, devCommentfs, devLabels, saveOut, saveIn):
    ...
    # model parameters
    learningRates = [0.01] * 3 + [0.005] * 2 + [0.003] * 5 + [0.002] * 10 + [0.001] * 10
    lstmUnits = 64
    dropoutKeepProb = 0.9
    numClasses = 2
    epochs = 30
    ...
    x = tf.placeholder(tf.int32, [batchSize, MAXDOCLENGTH])
...
def loadComments(filename, vocab, numBatches, batchSize):
    ...
    with open(filename, "r") as inFile:
        ...
        for i, line in enumerate(inFile, 1):
            ...
            commentsBatch.append(pad(commentTokenized, MAXDOCLENGTH))
